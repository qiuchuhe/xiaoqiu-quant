...
# 财联社电报API
CLS_API = "https://www.cls.cn/nodeapi/updateTelegraphList"
CLS_HEADERS = {
    "User-Agent": UA,
...
# 雪球热帖API
XUEQIU_HOT_API = "https://xueqiu.com/statuses/hot/topic.json"


def fetch_cls_telegraph(count=50):
    """获取财联社电报——通过公开RSS接口"""
    try:
        url = f"https://www.cls.cn/api/telegraph/list?app=CailianpressWeb&os=web&sv=8.4.6&rn={count}&pn=1"
        ...
        for item in items:
            ...
            title = item.get("title", "") or str(item.get("content", ""))[:100]
...
def fetch_cls_depth(count=30):
    """财联社深度文章——通过文章列表API"""
    try:
        url = f"https://www.cls.cn/v3/depth/home/assembled?app=CailianpressWeb&os=web&sv=8.4.6&rn={count}"
        headers = {"User-Agent": UA, "Referer": "https://www.cls.cn/depth"}
        req = urllib.request.Request(url, headers=headers)
        with urllib.request.urlopen(req, timeout=15) as resp:
            data = json.loads(resp.read())
        ...
        for item in items:
            ts = item.get("ctime", 0)
            dt = datetime.fromtimestamp(ts).strftime("%H:%M") if ts else ""
            title = item.get("title", "") or str(item.get("brief", ""))[:100]
            title = re.sub(r'<[^>]+>', '', title)
            if title and len(title) > 5:
                news.append({
                    "time": dt,
...
def fetch_xueqiu_hot(count=20):
    ...
    try:
        ...
        # 先访问首页获取cookie
        req = urllib.request.Request("https://xueqiu.com", headers=headers)
        opener.open(req, timeout=10)
        # 再访问热门API
        req = urllib.request.Request(
            f"https://xueqiu.com/statuses/hot/listV2.json?since_id=-1&size={count}",
            headers=headers
        )
        ...
        for item in items:
            ...
            dt = datetime.fromtimestamp(ts/1000).strftime("%m-%d %H:%M") if ts > 1e10 else datetime.fromtimestamp(ts).strftime("%H:%M")
            title = item.get("title", "") or item.get("text", "") or item.get("description", "")[:120]
            title = re.sub(r'<[^>]+>', '', title)
            if title and len(title) > 5:
                news.append({
                    "time": dt,
...
def fetch_eastmoney_yaowen(count=20):
    """获取东方财富24小时滚动新闻"""
    try:
        ...
        # 从页面提取新闻标题
        items = re.findall(r'<a[^>]*href="(/a/[^"]+)"[^>]*title="([^"]+)"[^>]*>([^<]+)</a>', html)
        news = []
        for href, title, text in items[:count]:
            if len(title) > 5:
                news.append({
                    "time": "",
                    "title": title[:150],
    ...
    # 备用: 东财RSS
    try:
        url = "https://rss.eastmoney.com/News/Rss?code=yw"
        req = urllib.request.Request(url, headers={"User-Agent": UA})
        with urllib.request.urlopen(req, timeout=12) as resp:
            xml = resp.read().decode('utf-8', errors='replace')
        items = re.findall(r'<item>.*?<title>(.+?)</title>.*?<pubDate>(.+?)</pubDate>.*?</item>', xml, re.DOTALL)
        news = []
        for title, pubdate in items[:count]:
            title = re.sub(r'<!\[CDATA\[|\]\]>', '', title)
            if len(title) > 5:
                news.append({
                    "time": pubdate[-12:-5] if len(pubdate) > 10 else pubdate,
                    "title": title[:150],
                    "source": "东方财富RSS",
                })
        return news
...
def fetch_163_money_news(count=20):
    """获取网易财经新闻"""
    try:
        url = "https://money.163.com/special/00251K5B/news_json.js"
        headers = {"User-Agent": UA}
        req = urllib.request.Request(url, headers=headers)
        with urllib.request.urlopen(req, timeout=12) as resp:
            raw = resp.read().decode('gbk', errors='replace')
        # 格式: var news = [...]
        items = json.loads(re.search(r'\[.*\]', raw, re.DOTALL).group(0))
        news = []
...
def get_news():
    ...
    print(f"      新浪: {len(sina)}条")

    # 华尔街见闻 A股+全球+商品
    for ch, label in WSCN_CHANNELS:
        print(f"    抓取华尔街见闻·{label}...")
        wscn = get_wscn_news(ch, 10)
        all_news.extend(wscn)
        print(f"      {label}: {len(wscn)}条")

    # 财联社电报（盘后继续抓到晚上）
    print("    抓取财联社电报...")
    cls = fetch_cls_telegraph(50)
    all_news.extend(cls)
    print(f"      财联社: {len(cls)}条")
    # 财联社深度文章
    cls2 = fetch_cls_depth(30)
    all_news.extend(cls2)
    print(f"      财联社深度: {len(cls2)}条")

    # 东方财富要闻
    print("    抓取东方财富要闻...")
    em = fetch_eastmoney_yaowen(20)
    all_news.extend(em)
    print(f"      东方财富: {len(em)}条")

    # 雪球热门
    print("    抓取雪球热门...")
    xq = fetch_xueqiu_hot(20)
    all_news.extend(xq)
    print(f"      雪球: {len(xq)}条")

    # 网易财经
    print("    抓取网易财经...")
    wy = fetch_163_money_news(20)
    all_news.extend(wy)
    print(f"      网易: {len(wy)}条")
