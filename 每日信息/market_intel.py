...
def fetch_cls_telegraph(count=50):
    ...
    try:
        ...
        req = urllib.request.Request(CLS_API, data=payload, headers=CLS_HEADERS)
...
def fetch_xueqiu_hot(count=20):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            # 提取xq_a_token
            token_match = re.search(r'xq_a_token=([^;]+)', cookies)
            if token_match:
                token = token_match.group(1)
                headers["Cookie"] = f"xq_a_token={token}"

        req = urllib.request.Request(XUEQIU_HOT_API, headers=headers)
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read())
        items = data.get("list", [])[:count]
        news = []
        for item in items:
            ts = item.get("created_at", 0)
            dt = datetime.fromtimestamp(ts/1000).strftime("%H:%M") if ts > 10000000000 else datetime.fromtimestamp(ts).strftime("%m-%d %H:%M")
...
def fetch_eastmoney_yaowen(count=20):
    """获取东方财富要闻"""
    try:
        url = f"https://np-listapi.eastmoney.com/comm/web/getNewsByColumn?columnCode=yw&pageNo=1&pageSize={count}"
        ...
        for item in items:
            ...
            news.append({
                "time": ts[-8:-3] if len(ts) > 8 else ts,
                "title": title,
                "url": item.get("artUrl", ""),
                "source": "东方财富要闻",
            })
