...
def menu_strategy1():
    ...
    print(f"  1. 盘后全量扫描（生成明日候选）")
    print(f"  2. 盘中实时监控（5分钟刷新）")
    print(f"  3. 单次快扫")
    choice = input("  > ").strip()
...
╠══════════════════════════════════════════════╣
║                                              ║
║  {C.Y}1.{C.Z} 📊 策略一扫描（均线多头+温和放量）      ║
║  {C.Y}2.{C.Z} 🛡️  止损止盈预警守护                    ║
║  {C.Y}3.{C.Z} 📡 实时监控(xiaoqiu_monitor)            ║
║  {C.Y}4.{C.Z} 📈 行情看板                              ║
║  {C.Y}5.{C.Z} ⏪ 策略回测                                ║
║                                              ║
║  {C.D}q.{C.Z} 👋 退出                                    ║
...
💡 直接命令:
  {C.D}python 策略量化/策略1/scanner.py{C.Z}  策略一扫描
  {C.D}python stock_quant.py watch{C.Z}      查看自选股
  {C.D}python stock_quant.py backtest 600519{C.Z}  策略回测
""")
        ...
        if choice == "1":
            ...
            print(f"{C.R}不清楚选什么... 输入 1-5 或 q{C.Z}")
